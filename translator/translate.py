...
For each chapter:
  1. Extract paragraphs from the .md file (already clean, numbered by position)
...
Usage:
  python translate.py <book_dir> [--chapter N] [--batch-size N] [--dry-run]
...
def extract_paragraphs_from_md(md_path: Path) -> list[tuple[int, str]]:
    """
    Return [(para_id, text), ...] matching the p1, p2, ... IDs in the HTML.
    Skips headings (# …), hr lines (---), blockquotes, lists — same logic as
    md_block_to_html in main.rs: only plain-text blocks get a para-block.
    """
    ...
    result = []
    for block in blocks:
        if _is_para_block(block):
            para_id += 1
            result.append((para_id, block))
        # non-para blocks (hr, heading, blockquote, list) don't get an id
    return result


def _is_para_block(block: str) -> bool:
    """Mirror of md_block_to_html's is_para logic in main.rs."""
    t = block.strip()
    if t == "---":
        return False
    if re.match(r"^#{1,6} ", t):
    ...
    if all(l.lstrip().startswith(">") for l in lines):
        return False
    if all(l.strip() == "" or l.lstrip().startswith("- ") for l in lines):
        return False
    if all(l.strip() == "" or re.match(r"^\d+\. ", l.lstrip()) for l in lines):
        return False
    if "|" in t and t.count("\n") >= 1 and "---" in t:
        return False
    return True
...
def patch_html(html_path: Path, translations: dict[int, dict]):
    ...
    for pid, data in translations.items():
        ...
        if trans_p is not None:
            trans_p.string = data["translation"]
        if vocab_p is not None:
            vocab_p.string = data["vocab"]
        if chunks_p is not None:
            chunks_p.string = data["chunks"]
        changed += 1
...
def translate_chapter(client: anthropic.Anthropic, prompt_txt: str,
    ...
    md_path   = book_dir / f"chapter{chapter_num}.md"
    ...
    paragraphs = extract_paragraphs_from_md(md_path)
    if not paragraphs:
    ...
    if not todo:
        print("  already complete, patching html …", flush=True)
